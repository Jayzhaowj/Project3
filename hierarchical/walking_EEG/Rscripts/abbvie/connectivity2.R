...
par(mfrow = c(1, 1), cex.main = 2, cex.lab = 1.7)
xlim <- c(-0.5, 1.5)
ylim <- c(0, 1)
loc <- c(0, 1)
main <- paste0(type, " band (1-4Hz) of coh between ", region, " and rest of clusters") 
plot(x_coord[which(x_coord==-0.5):which(x_coord==1.5)], 
     band_delta$coh[suppl[1], which(x_coord==-0.5):which(x_coord==1.5)], 
     type = 'l', xlab = "time (s)", ylab = "squared coherence", yaxt = 'n', col = col[1],
     ylim = c(0, 1), main = main, cex.axis = 1.5)
...
legend(x = 1.2, y = 1.05, legend = cluster_area_abb[-region_ind], 
       col = col, lty = rep(1, length(suppl)), bty = 'n', cex = 1.7)
...
ylim <- c(0, 1)
loc <- c(0, 1)
main <- paste0(type, " band (4-8Hz) of coh between ", region, " and rest of clusters") 
plot(x_coord[which(x_coord==-0.5):which(x_coord==1.5)], 
...
index_sma <- cbind(index_all[, index_all[1, ] == 7], index_all[, index_all[2, ]==7])
for(i in 1:dim(index_sma)[2]){
  tmp_sd <- get_sd(sd$PDC, index_sma[1, i], index_sma[2, i], 4)[(P+1):(n_t - P),  ]
  ##sd_diff <- t(apply(tmp_sd, 1, function(x) x - tmp_sd[which(x_coord == -0.5), ]))
  png(paste0(plot_dir, cond_type, "/pdc_from_", cluster_area_abb[index_sma[2, i]], "_to_", cluster_area_abb[index_sma[1, i]], ".png"))
  plot.new()
  par(par(mar=c(5,6,4,1)+.1), cex.lab = 2, cex.axis = 1.5, cex.main = 2.5)
  .filled.contour(x_coord[which(x_coord==-0.5):which(x_coord==1.5)], 
                  y_coord, tmp_sd[which(x_coord==-0.5):which(x_coord==1.5), ], as.double(levels), col = jet.colors(length(levels)-1))
  dev.off()
  
  plot.title = {title(xlab="time (s)", ylab = "frequency (Hz)", main = bquote("PDC: "*.(cluster_area_abb[index_sma[2, i]])%->%.(cluster_area_abb[index_sma[1, i]])))
    abline(v=loc, col = 'black', lty = 2)},
...
for(i in 1:dim(index_sma)[2]){
  tmp_sd <- get_sd(sd$PDC, index_sma[1, i], index_sma[2, i], 4)[(P+1):(n_t - P),  ]
  sd_diff <- t(apply(tmp_sd, 1, function(x) x - tmp_sd[which(x_coord == -0.5), ]))
  png(paste0(plot_dir, cond_type, "/pdc_diff_from_", cluster_area_abb[index_sma[2, i]], "_to_", cluster_area_abb[index_sma[1, i]], ".png"))
  par(par(mar=c(5,6,4,1)+.1), cex.lab = 2, cex.axis = 1.5, cex.main = 2.5)
  filled.contour(x_coord[which(x_coord==-0.5):which(x_coord==1.5)], 
                  y_coord, sd_diff[which(x_coord==-0.5):which(x_coord==1.5), ],
                  plot.title = {title(xlab="time (s)", ylab = "frequency (Hz)", main = bquote("PDC: "*.(cluster_area_abb[index_sma[2, i]])%->%.(cluster_area_abb[index_sma[1, i]])))
                    abline(v=loc, col = 'black', lty = 2)},
                  color.palette = jet.colors, zlim = c(0, 1))
